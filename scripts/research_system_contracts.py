...
def main() -> int:
    ...
    try:
        ...
        baby_solver._JUDGE_FEEDBACK_JOURNAL.clear()
        ...
        recorded_rejection = json.loads(baby_solver.judge_feedback_json())
    finally:
    ...
    checks["decisive_judge_rejections_are_deduplicated_per_solve"] = (
        ...
        and recorded_rejection["attempt_count"] == 1
        and recorded_rejection["attempts"][0]["route"] == "contract:first"
    )
    ...
    synthetic_grind = "\n".join([
        "Submission.lean:55:2: error: `grind` failed",
        "case grind",
        "G : Type",
        "⊢ False",
        "[grind] Goal diagnostics",
        "  [facts] Asserted facts",
        *[f"    [prop] generated_fact_{index}" for index in range(1200)],
        "  [limits] Thresholds reached",
        "    [limit] maximum number of instances generated by E-matching has been reached, threshold: `(instances := 1000)`",
    ])
    baby_solver._JUDGE_FEEDBACK_JOURNAL.clear()
    baby_solver.record_judge_feedback(
        "contract:large_grind",
        "true",
        "intro G _ h\nintro x\nhave sat1 := h x x\ngrind",
        {"status": "incorrect", "stderr": synthetic_grind},
        source="contract",
    )
    compact_journal_text = baby_solver.judge_feedback_json()
    compact_journal = json.loads(compact_journal_text)
    compact_context = baby_solver.llm_context(
        baby_solver.parse_equation("x = x"),
        baby_solver.parse_equation("x = y"),
        [],
        "contract",
    ...
    baby_solver._JUDGE_FEEDBACK_JOURNAL.clear()
    compact_attempt = compact_journal["attempts"][0]
    checks["solver_owned_judge_history_is_compact_and_informative"] = (
        "{history.attempts}" not in baby_solver.PROMPT
        and "{solver.judge_feedback}" in baby_solver.PROMPT
        and len(compact_journal_text) < 6000
        and "generated_fact_1199" not in compact_journal_text
        and compact_attempt["failure_class"] == "grind_instance_limit"
        and compact_attempt["unresolved_goals"] == ["False"]
        and "instances := 1000" in compact_attempt["limits_reached"][0]
        and json.loads(compact_context["judge_feedback"])["attempt_count"] == 1
    )
    ...
    hard3_0202_h = baby_solver.parse_equation(
        "x = (x ◇ (y ◇ z)) ◇ (y ◇ w)"
    )
    hard3_0202_g = baby_solver.parse_equation(
        "x ◇ y = x ◇ ((y ◇ z) ◇ x)"
    )
    original_hint_payload_attempt = baby_solver.hint_payload_attempt
    deep_routes: list[str] = []
    continuation_actions: list[dict[str, object]] = []
    continuation_body = "intro x y z\nexact h x y z x"
    try:
        def fake_deep_judge(route, body, **kwargs):
            del body, kwargs
            deep_routes.append(route)
            if route.endswith("structured_continuation"):
                return {"status": "accepted"}
            return {
                "status": "incorrect",
                "stderr": (
                    "verbose grind state " * 100
                    + "maximum number of instances generated by E-matching "
                    + "has been reached, threshold: (instances := 1000)"
                ),
            }

        def fake_hint_payload_attempt(action, h_eq, g_eq, **kwargs):
            del h_eq, g_eq, kwargs
            continuation_actions.append(action)
            return continuation_body, {"status": "body_built"}

        baby_solver.judge_true_attributed = fake_deep_judge
        baby_solver.hint_payload_attempt = fake_hint_payload_attempt
        deep_body, deep_state = baby_solver.run_tool_call_detailed(
            {"kind": "tool_call", "tool": "deep_saturation", "target": "goal"},
            hard3_0202_h,
            hard3_0202_g,
            verify_candidates=True,
        )
        ...
        baby_solver.judge_true_attributed = original_judge_true_attributed
        baby_solver.hint_payload_attempt = original_hint_payload_attempt
    checks["deep_saturation_prefers_structured_continuation"] = (
        deep_body == continuation_body
        and deep_state is not None
        and deep_state.get("status") == "proved"
        and deep_state.get("already_judged_accepted") is True
        and deep_state.get("accepted_route") == "structured_continuation"
        and deep_routes == ["llm:tool:deep_saturation:structured_continuation"]
        and len(continuation_actions) == 1
        and continuation_actions[0].get("lemma")
        == "a ◇ ((b ◇ c) ◇ d) = a ◇ b"
    )
    checks["judge_diagnostic_keeps_actionable_tail"] = (
        "maximum number of instances" in baby_solver.compact_judge_diagnostic({
            "stderr": (
                "verbose grind state " * 100
                + "maximum number of instances generated by E-matching"
            )
    ...
    unverified_deep_body, unverified_deep_state = baby_solver.run_tool_call_detailed(
        {"kind": "tool_call", "tool": "deep_saturation", "target": "goal"},
        hard3_0202_h,
        hard3_0202_g,
        verify_candidates=False,
    )
    checks["deep_saturation_generation_is_not_proof"] = (
        unverified_deep_body is not None
