...
def main() -> int:
    ...
    checks["deep_saturation_rejection_uses_structured_continuation"] = (
        ...
        and len(deep_routes) == 2
        and len(continuation_actions) == 1
        and continuation_actions[0].get("lemma")
        == "a ◇ ((b ◇ c) ◇ d) = a ◇ b"
        and "maximum number of instances" in (
            deep_state.get("judge_attempts", [{}])[0].get("diagnostic", "")
        )
    )
