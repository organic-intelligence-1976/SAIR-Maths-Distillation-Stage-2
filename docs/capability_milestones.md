...
| Hard false-case consumption | With the LLM already selecting reasonable routes, System 1 can turn at least one previously unsolved hard false focus case into an accepted countermodel under fixed budget. | B+ | `hard2_0093` has an accepted size-6 countermodel from `local_search:n=6:seed=2`; `hard2_0125` is now accepted by `cp_sat:n=6`; `hard2_0027` remains the next frontier. | Stop this B+ surface; define a stricter A-/next-frontier target before more false-consumer work. |
